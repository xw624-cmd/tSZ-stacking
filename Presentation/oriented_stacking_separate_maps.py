...
STACK_FULL_GRID_BOTTOM = 0.15

STACK_FULL_GRID_TOP = 0.79
...
CAP_FIT_SECTOR_SUPTITLE = '{\\rm Oriented-stack\\ CAP\\ ' + _CAP_FIT_MODEL_TEX + '\\ Fit:\\ ML\\ Curve}'

CACHE_FILE = os.path.join(CACHE_DIR, 'stamps.h5')

ORIENTED_PDF_NAMES = ['summary_unoriented_full_stack_1x3.pdf', 'summary_oriented_full_stack_1x3.pdf', 'summary_oriented_full_stack_cap_profiles_1x3.pdf', 'summary_oriented_sector_cap_correlation_2x3.pdf', 'summary_oriented_sector_posterior_curves_1x3.pdf', 'summary_oriented_hist_ba_selected_1x3.pdf', 'summary_radio_full_stack_1x3.pdf']
...
def _plot_single_stack_type(
    ...
    """Plot one tightly cropped 1xN figure for an oriented or unoriented stack."""
    ...
    fig, axes = plt.subplots(
        ...
        ncols,
        figsize=(
            STACK_FULL_FIG_WIDTH_PER_COL * ncols,
            STACK_FULL_FIG_HEIGHT_PER_ROW,
        ),
        squeeze=False,
        sharex=True,
        sharey=True,
    ...
    grid_right = _set_touching_square_grid(
        fig,
        axes,
        STACK_FULL_GRID_LEFT,
        STACK_FULL_GRID_BOTTOM,
        STACK_FULL_GRID_TOP,
    )
    ...
    for c, bin_result in enumerate(all_bin_results):
        ...
        if isinstance(res, dict):
            stack = res.get(stack_key)
            pixscale = res.get("pixscale")
        ...
        im = _imshow_stack_panel(
            ax,
            stack,
            pixscale,
            stack_norm,
            n_success,
            axis_label_size=STACK_FULL_AXIS_LABEL_SIZE,
            tick_label_size=STACK_FULL_TICK_LABEL_SIZE,
            n_label_size=STACK_FULL_N_LABEL_SIZE,
        )
        ...
        ax.set_title(
            ...
            fontsize=STACK_FULL_PANEL_TITLE_SIZE,
            pad=STACK_FULL_PANEL_TITLE_PAD,
        )
    ...
    if last_im is not None:
        cax = fig.add_axes(
            [
                grid_right + STACK_COLORBAR_PAD,
                STACK_FULL_CBAR_BOTTOM,
                STACK_COLORBAR_WIDTH,
                STACK_FULL_CBAR_HEIGHT,
            ]
        )
        ...
        _format_colorbar(
            cb,
            STACK_COLORBAR_LABEL,
        )

    # -------------------------------------------------------------
    # Create the title, then center it over the actual visible content
    # rather than over the original uncropped figure canvas.
    # -------------------------------------------------------------

    title_artist = fig.suptitle(
        suptitle,
        fontsize=STACK_FULL_SUPTITLE_SIZE,
        x=0.5,
        y=STACK_FULL_SUPTITLE_Y,
        ha="center",
    )

    # Matplotlib must draw the figure before tight bounding boxes are known.
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()

    # Include the three map axes and the colorbar axis, but not the suptitle.
    content_axes = list(axes)

    if cax is not None:
        content_axes.append(cax)

    content_bboxes = []

    for content_ax in content_axes:
        bbox = content_ax.get_tightbbox(renderer)

        if bbox is not None:
            content_bboxes.append(bbox)

    if content_bboxes:
        content_bbox = matplotlib.transforms.Bbox.union(content_bboxes)

        # Horizontal center of everything that will remain after cropping.
        content_center_pixels = 0.5 * (
            content_bbox.x0 + content_bbox.x1
        )

        # Convert the pixel coordinate into figure coordinates.
        content_center_figure = fig.transFigure.inverted().transform(
            (content_center_pixels, 0.0)
        )[0]

        title_artist.set_x(content_center_figure)

    # Redraw after repositioning the title.
    fig.canvas.draw()

    # -------------------------------------------------------------
    # Save with a tight horizontal crop
    # -------------------------------------------------------------
    ...
    print(
        f"  [summary full stack: {stack_key}] {path}"
    )

def plot_summary_full_mass_stacks(all_bin_results, out_dir, stack_norm):
    """Save unoriented and oriented full-stack maps as separate 1x3 PDFs."""
    _plot_single_stack_type(
        all_bin_results,
        out_dir,
        stack_norm,
        stack_key='stack_unori',
        suptitle='{\\rm Unoriented\\ Stacked\\ Compton\\ }$y${\\rm\\ Maps}',
        filename='summary_unoriented_full_stack_1x3.pdf',
    )
    _plot_single_stack_type(
        all_bin_results,
        out_dir,
        stack_norm,
        stack_key='stack_ori',
        suptitle='{\\rm Oriented\\ Stacked\\ Compton\\ }$y${\\rm\\ Maps}',
        filename='summary_oriented_full_stack_1x3.pdf',
    )
...
def plot_summary_sector_cap_profiles(all_bin_results, out_dir):
    ...
    for c, bin_result in enumerate(all_bin_results):
        ...
        if maj_m is None or maj_s is None or min_m is None or (min_s is None):
            ...
            ax.errorbar(CAP_RADII_ARCMIN - 0.04, maj_m, yerr=maj_s, fmt='P', linestyle='none', color=CAP_MAJOR_COLOR, capsize=CAP_ERROR_CAPSIZE, lw=CAP_ERROR_LW, ms=CAP_ERROR_MARKER_SIZE, label=CAP_MAJOR_SECTOR_LABEL)
            ax.errorbar(CAP_RADII_ARCMIN + 0.04, min_m, yerr=min_s, fmt='X', linestyle='none', color=CAP_MINOR_COLOR, capsize=CAP_ERROR_CAPSIZE, lw=CAP_ERROR_LW, ms=CAP_ERROR_MARKER_SIZE, label=CAP_MINOR_SECTOR_LABEL)
