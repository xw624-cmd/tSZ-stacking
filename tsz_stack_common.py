...
# Stellar-age 4x3 typography.
# This figure is much more cramped than the 2x3 figure, so keep it separate.
STACK_AGE_PANEL_TITLE_SIZE = 12
STACK_AGE_PANEL_TITLE_PAD = 18
STACK_AGE_SUPTITLE_SIZE = 16
STACK_AGE_AXIS_LABEL_SIZE = 15
STACK_AGE_TICK_LABEL_SIZE = 12
STACK_AGE_N_LABEL_SIZE = 9
STACK_AGE_ROW_LABEL_SIZE = 14

# Radio 1x3 typography.
...
HIST_BA_X_LABEL = r"$b/a$"
HIST_SELECTED_BA_SUPTITLE = r"{\rm Oriented\ Sample\ Axis\ Ratio\ Distribution}"
...
def _imshow_stack_panel(
    ...
    if n_success is not None:
        ax.text(
            ...
            bbox=dict(
                facecolor="white",
                alpha=STACK_N_LABEL_BBOX_ALPHA,
                edgecolor="none",
                pad=STACK_N_LABEL_BBOX_PAD,
            ),
        )
...
def plot_summary_full_mass_stacks(all_bin_results, out_dir, stack_norm):
    ...
    for r, (stack_key, row_label) in enumerate(row_defs):
        ...
        axes[r, 0].text(
            ...
            va="center",
            rotation=90,
            fontsize=STACK_FULL_ROW_LABEL_SIZE,
        )
...
def plot_summary_age_split_stacks(all_bin_results, out_dir, stack_norm):
    ...
    for r, (scheme_key, split_key, row_label) in enumerate(row_defs):
        for c, bin_result in enumerate(all_bin_results):
            ...
            im = _imshow_stack_panel(
                ...
                stack_norm,
                res.get("n_success", 0),
                axis_label_size=STACK_AGE_AXIS_LABEL_SIZE,
                tick_label_size=STACK_AGE_TICK_LABEL_SIZE,
                n_label_size=STACK_AGE_N_LABEL_SIZE,
            )
            if im is not None:
                last_im = im
            ...
            if r == 0:
                ax.set_title(
                    _mass_bin_label(mass_lo, mass_hi),
                    fontsize=STACK_AGE_PANEL_TITLE_SIZE,
                    pad=STACK_AGE_PANEL_TITLE_PAD,
                )
        ...
        axes[r, 0].text(
            ...
            va="center",
            rotation=90,
            fontsize=STACK_AGE_ROW_LABEL_SIZE,
        )
    ...
    fig.suptitle(
        STACK_AGE_SUPTITLE,
        fontsize=STACK_AGE_SUPTITLE_SIZE,
        y=STACK_AGE_SUPTITLE_Y,
    )
...
def plot_summary_age_split_cap_profiles(all_bin_results, out_dir):
    ...
    for c, bin_result in enumerate(all_bin_results):
        ...
        if c == 0:
            _set_cap_ylabel(ax, _cap_plot_axis_label(), CAP_AXIS_LABEL_SIZE_AGE)
        else:
            ax.tick_params(labelleft=False)
    ...
    fig.suptitle(CAP_AGE_SUPTITLE, fontsize=CAP_SUPTITLE_SIZE, y=CAP_AGE_SUPTITLE_Y)
...
def plot_summary_radio_full_stacks(all_bin_results, out_dir, stack_norm):
    ...
    for c, bin_result in enumerate(all_bin_results):
        ...
        im = _imshow_stack_panel(
            ...
            stack_norm,
            res.get("n_success", 0),
            axis_label_size=STACK_RADIO_AXIS_LABEL_SIZE,
            tick_label_size=STACK_RADIO_TICK_LABEL_SIZE,
            n_label_size=STACK_RADIO_N_LABEL_SIZE,
        )
        if im is not None:
            last_im = im
        ...
        ax.set_title(
            _mass_bin_label(mass_lo, mass_hi),
            fontsize=STACK_RADIO_PANEL_TITLE_SIZE,
            pad=STACK_RADIO_PANEL_TITLE_PAD,
        )
    ...
    fig.suptitle(
        STACK_RADIO_SUPTITLE,
        fontsize=STACK_RADIO_SUPTITLE_SIZE,
        y=STACK_RADIO_SUPTITLE_Y,
    )
...
def build_selection_and_cache():
    ...
    if USE_PHOTO_SHAPE:
        ...
        shape_valid_mask = valid_shape_ff & finite_shape_mask & (ab_ff < BA_MAX)
        print(f"  valid photo shape and 0 < selected b/a < {BA_MAX:.3f}: {(shape_valid_mask & broad_mask).sum():,}")

        shape_mask = shape_valid_mask
    else:
        shape_valid_mask = np.ones(n_firefly, dtype=bool)
...
# Cumulative cut-flow table.
    cum_photo = broad_mask & shape_valid_mask

    # Map footprint chains off whichever mask precedes it given the switches.
    if USE_PHOTO_SHAPE:
        pre_map = cum_photo
    else:
        pre_map = broad_mask
    cum_map = pre_map & inside_map
    ...
    # Radio stage only appears when a radio selection is actually in effect.
    apply_radio_stage = (FIRST_PATH is not None) and (RADIO_ONLY or EXCLUDE_RADIO)
    if apply_radio_stage:
        _s += 1
        if EXCLUDE_RADIO:
            cum_radio = cum_map & ~has_radio_ff
            cut_stages.append(
                (f"{_s}. FIRST radio removal ({FIRST_MATCH_ARCSEC:.0f} arcsec)", cum_radio)
            )
            final_label = "FINAL non-radio sample"
        else:  # RADIO_ONLY
            cum_radio = cum_map & has_radio_ff
            cut_stages.append(
                (f"{_s}. FIRST radio-only keep ({FIRST_MATCH_ARCSEC:.0f} arcsec)", cum_radio)
            )
            final_label = "FINAL radio-only sample"
    else:
        final_label = "FINAL sample (no radio cut)"

    print_cumulative_cut_table(cut_stages, logm_ff, final_label=final_label)
...
def plot_summary_age_split_weighted_mass_histograms(all_bin_results, h5f, out_dir):
    ...
    for c, bin_result in enumerate(all_bin_results):
        ...
        if c == 0:
            _set_hist_ylabel(ax, HIST_WEIGHTED_FRACTION_Y_LABEL)
        else:
            ax.tick_params(labelleft=False)
...
def plot_summary_age_split_mass_weight_values(all_bin_results, h5f, out_dir):
    ...
    for c, bin_result in enumerate(all_bin_results):
        ...
        ax.set_xlabel(_hist_xlabel("logm"), fontsize=HIST_AXIS_LABEL_SIZE)
        if c == 0:
            _set_hist_ylabel(ax, HIST_MEAN_MASS_WEIGHT_Y_LABEL)
        else:
            ax.tick_params(labelleft=False)
        _prune_touching_x_ticks(ax)
        ax.legend(loc=HIST_LEGEND_LOC, fontsize=HIST_LEGEND_SIZE)
...
def plot_summary_oriented_selected_ba_histograms(all_bin_results, h5f, out_dir):
    ...
    for c, bin_result in enumerate(all_bin_results):
        ...
        ax.set_title(
            _mass_bin_label(mass_lo, mass_hi),
            fontsize=HIST_PANEL_TITLE_SIZE,
            pad=HIST_PANEL_TITLE_PAD,
        )
        ax.tick_params(labelsize=HIST_TICK_LABEL_SIZE)
    ...
    fig.suptitle(
        HIST_SELECTED_BA_SUPTITLE,
        fontsize=HIST_SUPTITLE_SIZE,
        y=HIST_SUPTITLE_Y,
    )
...
if __name__ == "__main__":
    raise SystemExit(
        "This is the shared helper module. Run oriented_stack_only.py "
        "or stellar_age_stack_only.py instead."
    )